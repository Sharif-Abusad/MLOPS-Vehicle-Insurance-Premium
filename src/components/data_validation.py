...
class DataValidation:
    ...
    def is_column_exist(self, df:DataFrame):
        ...
        try:
            ...
            missing_numerical_coolumns = []
            missing_categorical_coolumns = []

            for column in self._schema_config["numerical_columns"]:
                if column not in dataframe_columns:
                    missing_numerical_coolumns.append(column)

            if len(missing_numerical_coolumns) > 0:
                logging.info(f"Missing numerical column: {missing_numerical_coolumns}")

            for column in self._schema_config["categorical_columns"]:
                if column not in dataframe_columns:
                    missing_categorical_coolumns.append(column)

            if len(missing_categorical_coolumns) > 0:
                logging.info(f"Missing categorical column: {missing_categorical_coolumns}")

            return False if len(missing_categorical_coolumns) > 0 or len(missing_numerical_coolumns) > 0 else True
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                ...
                logging.info(f"All categorical/numerical columns present in training dataframe: {status}")

            status = self.is_column_exist(df=test_df)
            if not status:
                validation_error_msg += f"Columns are missing in test dataframe."
            else:
                logging.info(f"All categorical/numerical columns present in testing dataframe: {status}")
